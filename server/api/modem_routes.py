...
from server.api.frontend_routes import REQUEST_ID_SIZE, g_requestTracker
endpoints = Blueprint("endpoints", __name__)

@endpoints.route("/ACK", methods=["POST"])
def modem_ack():
    """
...
@endpoints.route("/sensorData", methods=["POST"])
...
@endpoints.route("/battery", methods=["POST"])
def modem_post_battery_status():
    ...
    g_requestTracker.updateRequest(requestID=request_id, status="Battery Status Received")
    return {
        "status": "success",
        "message": "successfully sent sensor data",
    }